...
# Set up save directory for all data
save_directory = r"C:\Users\BCPLACE\Documents\MEGA\WI Studium\3. Semester\Unternehmenssoftware\AKI-Trading-Bot\data\raw"
...
def fetch_and_save_crypto_data(symbol, since_days=90, interval="1h"):
    ...
    save_path = os.path.join(save_directory, f"{symbol_filename}_data.csv")
    df.to_csv(save_path, index=False)
    print(f"Crypto data for {symbol} saved to {save_path}")

# Function to fetch and save macro data from Yahoo Finance
def fetch_and_save_macro_data(ticker, name, period="5y", interval="1d"):
    print(f"Fetching data for {name} ({ticker})...")
    data = yf.download(ticker, period=period, interval=interval)
    if not data.empty:
        save_path = os.path.join(save_directory, f"{name}_data.csv")
