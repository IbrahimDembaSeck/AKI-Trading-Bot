...
y_test_rescaled = sc.inverse_transform(
    np.concatenate([y_test.numpy().reshape(-1, 1), np.zeros((y_test.shape[0], features.shape[1] - 1))], axis=1))[:, 0]
y_pred_rescaled = sc.inverse_transform(
    np.concatenate([y_pred_test.numpy().reshape(-1, 1), np.zeros((y_pred_test.shape[0], features.shape[1] - 1))],
                   axis=1))[:, 0]
...
# 7. Monte Carlo Simulation with increased randomness
num_simulations = 100  # Number of random paths
num_weeks_to_predict = len(y_test_rescaled)
last_price = y_pred_rescaled[-1]

# Increased randomness settings
mean_return = 0.0002  # Increased the drift factor to add randomness
volatility = 0.02  # Increased the volatility to simulate more price variations
...
# Expected path from Monte Carlo
expected_path = np.mean(monte_carlo_paths, axis=0)

# 8. Visualization of Monte Carlo Simulations
plt.figure(figsize=(16, 8))
...
for i in range(num_simulations):
    plt.plot(monte_carlo_paths[i], color='grey', alpha=0.1)

# Plot expected Monte Carlo path
plt.plot(expected_path, color='red', label='Monte Carlo Expected Price', linewidth=2)

plt.title('Monte Carlo Simulation for Bitcoin Prices with Increased Randomness')
plt.xlabel('Weeks')
plt.ylabel('Price in USD')
plt.legend()
plt.show()
